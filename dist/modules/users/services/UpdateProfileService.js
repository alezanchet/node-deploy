...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const userWithUtdatedEmail = await this.usersRepository.findByEmail(email);

    if (userWithUtdatedEmail && userWithUtdatedEmail.id !== user.id) {
      throw new _AppError.default('E-mail alreadt in use.');
    }
